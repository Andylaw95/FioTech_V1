...
Wire format observed on /dev/tty485_1 @ 9600 8N1 (NOVOX customised firmware):
    ...
    └─ 15-digit IMEI/serial   └─ 64 floats = PPV samples (mm/s)
...
def _build_payload(device_id, device_name, serial_id, peak, peak_n,
                   line_count):
    if peak is None:
        return None
    abs_peak = abs(peak)
        ...
        try:
            ...
            if raw:
                ...
                if sid and vals:
                    last_serial = sid
                    line_count += 1
                    line_max = max(abs(v) for v in vals)
            ...
            if now - window_start >= upload_window_s:
                if peak is not None:
                    payload = _build_payload(device_id, device_name,
                                             last_serial, peak, peak_n,
                                             line_count)
                    try:
                        upload_fn(payload)
                        log("[AS400] upload ppv_max=%.4f mm/s alarm=%d lines=%d"
                            % (peak, _alarm_level(peak), line_count))
