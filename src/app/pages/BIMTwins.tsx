      ...
      try {
        ...
        devices.forEach((d, idx) => {
          ...
          if (reading?.decoded) {
            ...
            } else if (d.type === 'Noise' && decoded.tvoc != null) {
              newTelemetry[d.id] = decoded.tvoc;
            } else {
...
export function BIMTwins() {
  ...
  useEffect(() => {
    ...
    api.getPropertyTelemetry(selectedPropertyId)
      ...
      .catch(() => setConnectionStatus('connected')); // Still show connected since property is accessible
    return () => {};
  ...
  const devicePositions = useMemo(() => {
    return devices.map((d) => {
      ...
      for (let c = 0; c < d.id.length; c++) hash = ((hash << 5) - hash) + d.id.charCodeAt(c);
      hash = Math.abs(hash);
      const zoneIdx = hash % TABLE_ZONES.length;
      const zone = TABLE_ZONES[zoneIdx];
      const spreadX = ((hash % 100) / 100 - 0.5) * zone.w * 0.6;
      const spreadY = (((hash >> 8) % 100) / 100 - 0.5) * zone.h * 0.6;
      return { device: d, floor: 0, tableZone: zone.id, x: zone.cx + spreadX, y: zone.cy + spreadY };
    });
  }, [devices, TABLE_ZONES]);
