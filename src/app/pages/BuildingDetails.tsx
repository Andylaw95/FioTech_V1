...
import {
  ...
  Volume2, Clock, ChevronDown, ChevronRight, Wifi, WifiOff
} from 'lucide-react';
...
// ── Tab definitions ──────────────────────────────────────
type TabId = 'overview' | 'devices' | 'locations' | 'bms' | 'alarms';
...
export function BuildingDetails() {
  ...
  // Group property devices by location for "By Location" view
  const devicesByLocation = useMemo(() => {
    ...
    // Sort: Unassigned last, then alphabetical
  ...
  // Toggle location group expand/collapse
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="border-b border-slate-200">
        <nav className="flex gap-1 sm:gap-2 overflow-x-auto -mb-px" aria-label="Tabs">
          {TABS.map(tab => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={clsx(
                  "flex items-center gap-2 px-3 sm:px-5 py-3 sm:py-3.5 text-sm sm:text-base font-medium border-b-2 transition-colors whitespace-nowrap",
                  isActive
              ...
              >
                ...
                {badgeCount > 0 && (
                  <span className={clsx(
                    "ml-1 inline-flex h-5 sm:h-6 min-w-[20px] sm:min-w-[24px] items-center justify-center rounded-full px-1.5 sm:px-2 text-xs font-bold",
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
              ...
              {zones.length > 0 ? (
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  {zones.map((zone) => (
                    <div key={zone.id} onClick={() => {
                      setActiveTab('locations');
                      setExpandedLocations(new Set([zone.name]));
                    }} className="flex items-center justify-between p-2.5 sm:p-3.5 rounded-xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50/50 transition-all cursor-pointer gap-2">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by device name, type, or location..."
                  value={sensorSearch}
                  onChange={(e) => setSensorSearch(e.target.value)}
                  className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                />
            ...
            {devicesByLocation.length > 0 ? (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className="text-slate-500 font-medium">{devicesByLocation.length} location{devicesByLocation.length !== 1 ? 's' : ''}</span>
                  <span className="text-slate-300">·</span>
                  <span className="text-slate-500">{property.devices?.length ?? 0} total devices</span>
                  <span className="text-slate-300">·</span>
                  <button
                    onClick={() => setExpandedLocations(new Set(devicesByLocation.map(([loc]) => loc)))}
                ...
                {/* Location groups */}
                {devicesByLocation.map(([location, devices]) => {
                  const isExpanded = expandedLocations.has(location);
                  const onlineCount = devices.filter(d => d.status === 'online').length;
                  const offlineCount = devices.filter(d => d.status === 'offline').length;
                  const warningCount = devices.filter(d => d.status === 'warning').length;
                  const lowBatteryCount = devices.filter(d => d.battery <= 15).length;
                  // Filter by search query within location
                  ...
                  return (
                    <div key={location} className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden transition-all">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          <MapPin className={clsx("h-4 w-4 shrink-0", location === 'Unassigned' ? 'text-slate-300' : 'text-blue-500')} />
                          <span className={clsx(
                            "font-semibold text-sm truncate",
                            location === 'Unassigned' ? 'text-slate-400 italic' : 'text-slate-900'
                          )}>
                            {location}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 ml-auto shrink-0">
                          ...
                          {offlineCount > 0 && (
                            <span className="flex items-center gap-1 text-xs font-medium text-red-700 bg-red-50 rounded-full px-2 py-0.5">
                              <WifiOff className="h-3 w-3" /> {offlineCount}
                            </span>
                          )}
                          {lowBatteryCount > 0 && (
                            <span className="flex items-center gap-1 text-xs font-medium text-orange-700 bg-orange-50 rounded-full px-2 py-0.5">
                              🔋 {lowBatteryCount} low
                            </span>
                          )}
                      ...
                      {isExpanded && (
                        <div className="border-t border-slate-100 px-4 py-3 sm:px-5 sm:py-4 bg-slate-50/50">
                          <PropertyDevicePanel
                            devices={searchFiltered}
                            propertyName={property.name}
                            onDeviceChange={() => { fetchProperty(); fetchAllDevices(); }}
                            searchQuery=""
                            deviceReadings={telemetry?.deviceReadings}
              ...
              <div className="flex flex-col items-center justify-center py-20 text-slate-400 rounded-2xl border-2 border-dashed border-slate-200 bg-white">
                ...
                <p className="text-base font-medium text-slate-500">No devices assigned</p>
                <p className="text-sm text-slate-400 mt-1">Assign devices with locations to see grouped view.</p>
                <button
                  onClick={() => navigate('/devices')}
                  className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-700 transition-colors"
                >
