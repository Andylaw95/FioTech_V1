...
function DeviceCard({ device, isExpanded, onToggleExpand, onStatusChange, liveReading }: DeviceCardProps) {
  ...
  return (
    <div className={clsx(
      ...
      >
        {/* Icon */}
        <div className={clsx("p-2.5 rounded-xl shrink-0", meta.bg, meta.color)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">{device.location} · {meta.label}</p>
        </div>
        ...
        <div className="border-t border-slate-100 bg-slate-50/50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="p-4 sm:p-6 space-y-5">
                ...
                {editingLocation ? (
                  <div className="flex items-center gap-1.5 flex-1">
                    <input
                      ...
                      className="h-8 flex-1 rounded-lg border border-blue-300 bg-white px-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-100 min-w-0"
                      disabled={locationSaving}
                    />
                    <button
                      onClick={handleLocationSave}
                      disabled={locationSaving}
                      className="flex items-center justify-center h-8 w-8 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors shrink-0"
                    >
                      {locationSaving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                    </button>
                    <button
                      onClick={handleLocationCancel}
                      disabled={locationSaving}
                      className="flex items-center justify-center h-8 w-8 rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors shrink-0"
                    >
                  ...
                  <button
                    onClick={() => { setLocationValue(device.location || ''); setEditingLocation(true); }}
                  ...
                  >
                    ...
                    <Pencil className="h-3 w-3 text-slate-300 group-hover:text-blue-400 shrink-0" />
                  </button>
                )}
              </div>
              <button
              ...
              <div className="rounded-lg bg-white border border-slate-200 p-3">
                ...
                <div className="flex items-center gap-1.5">
                  {device.battery > 50 ? <Battery className="h-4 w-4 text-emerald-500" /> :
                   device.battery > 20 ? <BatteryMedium className="h-4 w-4 text-amber-500" /> :
                   <BatteryLow className="h-4 w-4 text-red-500" />}
                  <span className="text-sm font-bold text-slate-900">{device.battery}%</span>
                </div>
              </div>
              <div className="rounded-lg bg-white border border-slate-200 p-3">
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-4 sm:p-5">
              <DeviceHistoryChart deviceId={device.id} deviceType={device.type} devEui={device.devEui} focusMetric={focusMetric} />
            </div>
          </div>
        </div>
      )}
