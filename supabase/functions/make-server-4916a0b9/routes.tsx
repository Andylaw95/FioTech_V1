...
function resolveTargetUser(auth: { userId: string; email: string }, c: any): string {
  ...
  if (forUser && forUser !== auth.userId) {
    if (!MASTER_EMAILS.has(auth.email.toLowerCase())) return auth.userId;
    return forUser;
  }
  return auth.userId;
}
...
export function registerRoutes(app: any) {
  ...
  app.get("/make-server-4916a0b9/properties/:id/telemetry", async (c: any) => {
    ...
    try {
      ...
      // Fallback: if user has no sensor data, try master's primary data
      if (sensorData.length === 0 && userId !== MASTER_USER_ID) {
        try {
          const mRaw = await cachedKvGet(`sensor_data_${MASTER_USER_ID}`);
          if (Array.isArray(mRaw) && mRaw.length > 0) { sensorData = mRaw; }
  ...
  app.get("/make-server-4916a0b9/dashboard-bundle", async (c: any) => {
    ...
    try {
      ...
      if (accountType !== "demo") {
        ...
        if (sensorData.length === 0 && userId !== MASTER_USER_ID) {
          try { const mRaw = await cachedKvGet(`sensor_data_${MASTER_USER_ID}`); if (Array.isArray(mRaw) && mRaw.length > 0) sensorData = mRaw; } catch {}
        }
  ...
  app.get("/make-server-4916a0b9/telemetry", async (c: any) => {
    ...
    try {
      ...
      if (accountType !== "demo") {
        ...
        // Fallback: if user has no sensor data, try master's mirrored data
        if (sensorData.length === 0 && userId !== MASTER_USER_ID) {
          try {
            const mRaw = await cachedKvGet(`sensor_data_${MASTER_USER_ID}`);
            if (Array.isArray(mRaw) && mRaw.length > 0) sensorData = mRaw;
        ...
        // No sensor data at all — fall through to simulated telemetry
        // (don't return empty for testing accounts; show simulated data instead)
      }
  ...
  app.get("/make-server-4916a0b9/webhook-debug", async (c: any) => {
    const auth = await requireAuth(c);
    if (auth instanceof Response) return auth;
    if (!(auth as any).email?.includes("master@")) return c.json({ error: "Admin only." }, 403);
    ...
    try {
      ...
      if (decodedData) {
        ...
        const alarmChecks = [
          ...
          { field: "pressure", threshold: 0, type: "Pressure Alert", desc: "Abnormal pressure detected", above: false },
  ...
  app.get("/make-server-4916a0b9/device-history/:devEui", async (c: any) => {
    ...
    try {
      ...
      // Fallback: if user has no sensor data for this devEui, try master's
      // mirrored data (master receives copies of all webhook uplinks)
      if (entries.length === 0 && userId !== MASTER_USER_ID) {
        try {
          const mKey = `sensor_data_${MASTER_USER_ID}`;
          const mRaw = await cachedKvGet(mKey);
          if (Array.isArray(mRaw)) {
            entries = mRaw.filter((e: any) => {
              if (!e.devEUI || e.eventType === "join" || e.eventType === "ack") return false;
              if (e.devEUI.toLowerCase() !== devEui) return false;
              const ts = new Date(e.receivedAt).getTime();
              return ts > cutoff;
            });
            if (entries.length > 0) console.log(`[device-history] Fallback to master data: ${entries.length} entries for ${devEui}`);
          }
        } catch { /* master data unavailable, continue with empty */ }
      }
      ...
      const formatted = sampled.map((p: any) => ({
        ...p,
        timeLabel: new Date(p.time).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }),
